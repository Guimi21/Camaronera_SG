    ...
    if ($method === 'GET') {
        ...
        $query = "SELECT 
                ...
                WHERE c.id_grupo_empresarial = (
                    SELECT id_grupo_empresarial FROM usuario WHERE id_usuario = :id_usuario
                )
                ORDER BY c.id_compania ASC";
        
        $companias = $qb->executeQuery($query, [':id_usuario' => $id_usuario], true);
        ...
        $result = $qb->executeQuery($query, [':id_compania' => $id_compania, ':id_usuario' => $id_usuario], false);
        ...
        $count = $qb->countRecords('compania',
            'nombre = :nombre AND id_grupo_empresarial = (SELECT id_grupo_empresarial FROM usuario WHERE id_usuario = :id_usuario) AND id_compania != :id_compania',
            [':nombre' => $nombre, ':id_usuario' => $id_usuario, ':id_compania' => $id_compania]);
        ...
        $qb->updateRecord('compania', [
            ...
            'estado' => $estado,
            'fecha_actualizacion' => date('Y-m-d H:i:s')
        ], 'id_compania = :id_compania', [':id_compania' => $id_compania]);
