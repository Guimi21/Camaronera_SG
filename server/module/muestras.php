...
try {
    ...
    // PUT - Actualizar muestra
        ...
        $muestraExists = $qb->countRecords('muestra',
            'id_muestra = :id_muestra AND id_compania = :id_compania',
            [':id_muestra' => $id_muestra, ':id_compania' => $id_compania]
        );
        ...
        $qb->updateRecord('muestra', $updates, 'id_muestra = :id_muestra', [':id_muestra' => $id_muestra]);

        // Actualizar consumos de balanceado (eliminar antiguos e insertar nuevos)
        $qb->deleteRecord('consumo_balanceado', 'id_muestra = :id_muestra', [':id_muestra' => $id_muestra]);
    ...
    // GET - Obtener muestras
        ...
        if ($id_muestra) {
            $query = "SELECT id_muestra FROM muestra WHERE id_muestra = :id_muestra AND id_compania = :id_compania";
            $existe = $qb->executeQuery($query, [':id_muestra' => $id_muestra, ':id_compania' => $id_compania], true);
            ...
            WHERE s.id_muestra = :id_muestra AND s.id_compania = :id_compania";

            $muestras = $qb->executeQuery($query, [':id_muestra' => $id_muestra, ':id_compania' => $id_compania], true);
            ...
            if (!empty($muestras)) {
                ...
                $balanceadoQuery = "SELECT tb.nombre, COALESCE(cb.cantidad, 0) as cantidad
                    ...
                    LEFT JOIN consumo_balanceado cb ON cb.id_muestra = :id_muestra AND cb.id_tipo_balanceado = tb.id_tipo_balanceado
                ...
                $balanceados = $qb->executeQuery($balanceadoQuery, 
                    [':id_muestra' => $id_muestra_val, ':id_muestra_val' => $id_muestra_val], true);
        ...
        // Caso 2: Contar muestras de un ciclo
            ...
            $total = $qb->countRecords('muestra', 'id_ciclo = :id_ciclo', [':id_ciclo' => $id_ciclo]);
        ...
        // Caso 3: Obtener promedio de incremento de peso
            ...
            FROM muestra 
            WHERE id_ciclo = :id_ciclo AND incremento_peso IS NOT NULL";

            $resultado = $qb->executeQuery($query, [':id_ciclo' => $id_ciclo], true);
            $data = $resultado[0] ?? [];
        ...
        // Caso 4: Obtener última muestra de un ciclo
            ...
            $muestras = $qb->executeQuery($query, [':id_ciclo' => $id_ciclo], true);
            ErrorHandler::sendSuccessResponse($muestras);
        ...
        // Caso 5: Obtener penúltima muestra de un ciclo
            ...
            INNER JOIN ciclo_productivo cp ON cp.id_ciclo = s.id_ciclo
            INNER JOIN piscina p ON cp.id_piscina = p.id_piscina
            WHERE s.id_ciclo = :id_ciclo AND s.id_compania = :id_compania
            ORDER BY s.fecha_muestra DESC, s.fecha_creacion DESC
            LIMIT 1 OFFSET 1";

            $muestras = $qb->executeQuery($query, 
                [':id_ciclo' => $id_ciclo, ':id_compania' => $id_compania], true);
            ...
            if (!empty($muestras)) {
                foreach ($muestras as $index => $muestra) {
                    ...
                    $balanceados = $qb->executeQuery($balanceadoQuery, 
                        [':id_muestra' => $id_muestra_val, ':id_compania' => $id_compania], true);
        ...
        // Caso 6: Obtener todas las muestras con filtros
            ...
            if ($id_compania) {
                $query .= " AND s.id_compania = :id_compania AND tb.id_compania = :id_compania";
                $params[':id_compania'] = $id_compania;
            }
