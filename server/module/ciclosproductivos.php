...
try {
    ...
    if ($method === 'POST' && isset($_GET['action']) && $_GET['action'] === 'upload_pdf') {
        ...
        $compania = $qb->getRecord('compania', 'id_compania = ' . PARAM_ID_COMPANIA_LITERAL, [PARAM_ID_COMPANIA_LITERAL => $idCompania], 'nombre');
    ...
    if ($method === 'PUT') {
        ...
        $cicloExists = $qb->countRecords('ciclo_productivo',
            'id_ciclo = ' . PARAM_ID_CICLO . ' AND id_compania = ' . PARAM_ID_COMPANIA_LITERAL,
    ...
    if ($method === 'GET') {
        ...
        if (isset($_GET['id_ciclo']) && !empty($_GET['id_ciclo'])) {
            ...
            $query = "
                ...
                WHERE cp.id_ciclo = " . PARAM_ID_CICLO . " AND cp.id_compania = " . PARAM_ID_COMPANIA_LITERAL . "";

            $ciclo = $qb->executeQuery($query, [PARAM_ID_CICLO => $id_ciclo, PARAM_ID_COMPANIA_LITERAL => $id_compania], false);
        ...
        $query = "
            ...
            INNER JOIN piscina p ON cp.id_piscina = p.id_piscina
            LEFT JOIN tipo_alimentacion ta ON cp.id_tipo_alimentacion = ta.id_tipo_alimentacion
            WHERE cp.id_compania = " . PARAM_ID_COMPANIA_LITERAL . "
            ORDER BY CASE WHEN cp.estado = 'EN_CURSO' THEN 0 ELSE 1 END, cp.fecha_siembra DESC";

        $ciclos = $qb->executeQuery($query, [PARAM_ID_COMPANIA_LITERAL => $id_compania], true);
